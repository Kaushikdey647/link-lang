...
- `scripts/index.py` gains `--strategy {english_query,qa_pair}` (default `english_query`) to select between the two registered (backend, chunker) plans.
- `pipeline/indexer.py::_e5_text()` prepends `"passage: "` to qa_pair text before embedding, per e5's asymmetric query/passage prefix convention — a future query-side embedder against this collection must prefix queries with `"query: "` to match.
